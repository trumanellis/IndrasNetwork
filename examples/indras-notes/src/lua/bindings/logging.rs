...
/// Register logging functions with the notes table
...
mod tests {
    ...
    fn test_log_simple() {
        ...
        lua.load(r#"notes.log.info("Hello from Lua")"#)
            .exec()
            .unwrap();
    }
    ...
    fn test_log_levels() {
        ...
        lua.load(
            r#"
            notes.log.trace("trace message")
            notes.log.debug("debug message")
            notes.log.info("info message")
            notes.log.warn("warn message")
            notes.log.error("error message")
        "#,
        )
        .exec()
        .unwrap();
