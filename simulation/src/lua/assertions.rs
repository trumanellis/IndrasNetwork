...
//! but with better error messages for simulation testing.

use mlua::{Lua, Result, Table, Value};

/// Register assertion helpers with the indras table
...
mod tests {
    ...
    fn test_assert_eq_fail() {
        let lua = setup_lua();
        let result = lua.load("indras.assert.eq(1, 2)").exec();
    ...
    fn test_assert_ne_pass() {
        let lua = setup_lua();
        lua.load("indras.assert.ne(1, 2)").exec().unwrap();
    }

    #[test]
    fn test_assert_gt_pass() {
        let lua = setup_lua();
        lua.load("indras.assert.gt(2, 1)").exec().unwrap();
    }

    #[test]
    fn test_assert_gt_fail() {
        let lua = setup_lua();
        let result = lua.load("indras.assert.gt(1, 2)").exec();
        assert!(result.is_err());
    }
    ...
    fn test_assert_true() {
        let lua = setup_lua();
        lua.load("indras.assert.true_(true)").exec().unwrap();
        let result = lua.load("indras.assert.true_(false)").exec();
        assert!(result.is_err());
    }
    ...
    fn test_assert_nil() {
        let lua = setup_lua();
        lua.load("indras.assert.nil_(nil)").exec().unwrap();
        let result = lua.load("indras.assert.nil_(1)").exec();
        assert!(result.is_err());
    }
    ...
    fn test_assert_len() {
        let lua = setup_lua();
        lua.load("indras.assert.len({1, 2, 3}, 3)").exec().unwrap();
        let result = lua.load("indras.assert.len({1, 2, 3}, 2)").exec();
        assert!(result.is_err());
    }
    ...
    fn test_assert_fail() {
        let lua = setup_lua();
        let result = lua.load("indras.assert.fail('intentional failure')").exec();
        assert!(result.is_err());
    }
