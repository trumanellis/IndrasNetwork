...
/// An Intention is a Tree Artifact (TreeType::Intention) representing a player's
...
///
/// ## Tree structure (labeled refs)
///
/// ```text
/// Intention tree (TreeType::Intention)
/// ├── ref label="description"                    → Message leaf (description text)
/// ├── ref label="proof:{submitter_hex}:{ts}"     → Attestation leaf (proof body)
/// ├── ref label="pledge:{from_hex}:{token_hex}"  → Token leaf (pledged by supporter)
/// └── metadata:
...
impl Intention {
    ...
    pub fn create<A: ArtifactStore, P: PayloadStore, T: AttentionStore>(
        ...
        let tree = vault.place_tree(TreeType::Intention, audience, now)?;
    ...
    pub fn unreleased_attention<A: ArtifactStore, P: PayloadStore, T: AttentionStore>(
        ...
        let released_timestamps: Vec<i64> = tree
            .metadata
            .keys()
            .filter_map(|k| {
    ...
    pub fn release_attention<A: ArtifactStore, P: PayloadStore, T: AttentionStore>(
        ...
        for window in windows {
            ...
            let mut token_artifact = vault
                ...
                .ok_or(VaultError::ArtifactNotFound)?;
            if let Artifact::Leaf(ref mut leaf) = token_artifact {
                leaf.blessing_history.push(BlessingRecord {
                    from: player,
            ...
            let mut intention_artifact = vault
                .get_artifact(&self.id)?
                .ok_or(VaultError::ArtifactNotFound)?;
            let tree = intention_artifact
                .as_tree_mut()
                .ok_or(VaultError::NotATree)?;
            tree.metadata.insert(key, proof_label.into_bytes());
            vault
                .artifact_store_mut()
...
mod tests {
    ...
    fn test_pledge_token() {
        ...
        let token = vault
            .place_leaf(b"token-data", "my-token".to_string(), None, LeafType::Token, 1000)
            .unwrap();
    ...
    fn test_unreleased_attention_and_release() {
        ...
        let other_id = vault.place_tree(TreeType::Story, vec![player_a()], 1000).unwrap().id;
        ...
        if let Artifact::Leaf(leaf) = &token {
            assert_eq!(leaf.blessing_history.len(), 1);
            assert_eq!(leaf.blessing_history[0].from, player_a());
            assert_eq!(leaf.blessing_history[0].quest_id, Some(intention.id));
        } else {
            panic!("Expected leaf artifact");
        }
    }
    ...
    fn test_release_pledged_tokens() {
        ...
        let token = vault
            .place_leaf(b"pledged-token", "pledge-tok".to_string(), None, LeafType::Token, 1000)
            .unwrap();
        intention.pledge_token(&mut vault, token.id).unwrap();
    ...
    fn test_submit_proof_folder() {
        ...
        let folder = vault
            .place_tree(TreeType::Collection, vec![player_a()], 1000)
            .unwrap();
