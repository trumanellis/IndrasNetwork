...
pub const INTERFACE_MEMBERS: TableDefinition<&[u8], &[u8]> = TableDefinition::new("interface_members");
...
pub const PENDING_DELIVERY: TableDefinition<&[u8], &[u8]> = TableDefinition::new("pending_delivery");
...
impl RedbStorage {
    ...
    fn init_tables(&self) -> Result<(), StorageError> {
        ...
        write_txn.open_table(PEER_REGISTRY)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(INTERFACES)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(INTERFACE_MEMBERS)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(SYNC_STATE)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(EVENT_INDEX)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(PENDING_DELIVERY)
            .map_err(|e| StorageError::Io(e.to_string()))?;
        write_txn.open_table(SNAPSHOTS)
            .map_err(|e| StorageError::Io(e.to_string()))?;
...
mod tests {
    ...
    fn test_scan_prefix() {
        ...
        storage.put(PEER_REGISTRY, b"user:charlie", b"data3").unwrap();
        storage.put(PEER_REGISTRY, b"group:admins", b"data4").unwrap();
