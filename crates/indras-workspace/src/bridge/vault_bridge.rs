...
pub fn create_seeded_vault() -> Result<VaultHandle, indras_artifacts::VaultError> {
    ...
    let zephyr_events = vec![
        AttentionSwitchEvent {
            version: 1,
            author: peers[1].player_id,
            seq: 0,
            from: None,
            to: Some(team_discussion_id.clone()),
            wall_time_ms: now + 300_000,
            prev: [0u8; 32],
            sig: vec![],
        },
        AttentionSwitchEvent {
