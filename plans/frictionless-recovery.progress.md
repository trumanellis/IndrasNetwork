...
- [~] C.6 — Shared-realm re-join **deferred as a network-layer design decision**. The `peer_verification` primitives (B.7) are ready; wiring them into every `IndrasNetwork::inner.add_member` call-site requires a policy call (silent reject / prompt / log-and-allow) that's worth surfacing to the user rather than picking autonomously. Separate one-function slice at the network layer.
