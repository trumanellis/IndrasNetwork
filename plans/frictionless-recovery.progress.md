...
- [~] C.4 — Publish-on-save hook **deferred**. Plumbing into `vault_manager` touches code paths (`vault/sync_all.rs`, the GC loop) that Plan/CLAUDE.md reserves for agent1's lane. Landing safely needs a cross-agent handoff or a carefully scoped extension point first.
- [~] C.5 — Recovery-time re-pull **deferred**. Same vault-manager dependency plus a new UI for the file-restore progress overlay. Primitive (`reconstruct_file`) is ready; wiring follows vault integration in C.4.
