...
- [x] C.6 — `peer_verification::gate_peer_via_home_roster` gates `poll_recovery_releases` and `assemble_and_authenticate` on the steward's device cert in *their own* home `DeviceRoster` (greenfield: cert required, fail closed, retry next poll on missing state). Revoked or forged releases silently drop.
- [x] **Network-layer admission gate.** `ConnectionNotify` now carries the sender's PQ verifying key + a Dilithium signature over a domain-separated canonical encoding. The inbox handler rejects unsigned / invalidly-signed notifies before creating any DM realm state. All three emission sites (initial connect, retry, reciprocal reply) sign before sending. New `test_connection_notify_signed_verify_roundtrip` test covers sign → serialise → verify, plus field-tamper and wrong-key rejection. 25/25 direct_connect tests pass.
